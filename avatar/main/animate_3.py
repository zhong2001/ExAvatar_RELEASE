...
def main():
    ...
    # tester.tri_feat = tester.model.module.human_gaussian.extract_tri_feature()
    # # get Gaussian assets
    # tester.geo_feat = tester.model.module.human_gaussian.geo_net(tester.tri_feat)
    # tester.mean_offset = tester.model.module.human_gaussian.mean_offset_net(tester.geo_feat)  # mean offset of Gaussians
    # tester.scale = tester.model.module.human_gaussian.scale_net(tester.geo_feat)  # scale of Gaussians
    # tester.rgb = tester.model.module.human_gaussian.rgb_net(tester.tri_feat)  # rgb of Gaussians
    ...
    frame_idx_list = sorted([int(x.split('/')[-1][:-5]) for x in
                             glob(osp.join(args.motion_path, 'smplx_optimized', 'smplx_params_smoothed', '*.json'))])
    render_shape = cv2.imread(osp.join(args.motion_path, 'frames', str(frame_idx_list[0]) + '.png')).shape[:2]
    video_out = cv2.VideoWriter(motion_name + '.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 30,
                                (render_shape[1] * 3, render_shape[0]))
    for frame_idx in tqdm(frame_idx_list):
        ...
        with open(
                osp.join(args.motion_path, 'smplx_optimized', 'smplx_params_smoothed', str(frame_idx) + '.json')) as f:
            smplx_param = {k: torch.FloatTensor(v).cuda().view(-1) for k, v in json.load(f).items()}

        # print("Camera Parameters:", cam_param)
        # print("Translation (trans):", smplx_param['trans'])
        # tri_feat = tester.model.module.human_gaussian.extract_tri_feature()
        # geo_feat = tester.model.module.human_gaussian.geo_net(tri_feat)
        # mean_offset = tester.model.module.human_gaussian.mean_offset_net(geo_feat)  # mean offset of Gaussians
        # scale = tester.model.module.human_gaussian.scale_net(geo_feat)  # scale of Gaussians
        # rgb = tester.model.module.human_gaussian.rgb_net(tri_feat)
        #tester.model.module.human_gaussian.tri_feat = tester.model.module.human_gaussian.extract_tri_feature()
        render_time = time.time()
        # forward
        with torch.no_grad():
            human_asset,  human_offset, mesh_neutral_pose = tester.model.module.human_gaussian(
                smplx_param, cam_param)
            human_render = tester.model.module.gaussian_renderer(human_asset, render_shape, cam_param)
        renders_time = time.time() - render_time
        fps1 = 1 / renders_time
        print(f"Frame {frame_idx}: FPS: {fps1:.2f}")
        # smplx mesh render
        ...
        output = tester.model.module.smplx_layer(global_orient=root_pose, body_pose=body_pose, jaw_pose=jaw_pose,
        ...
        mesh_render = render_mesh(mesh, smpl_x.face, cam_param,
                                  np.ones((render_shape[0], render_shape[1], 3), dtype=np.float32) * 255).astype(
            np.uint8)
        ...
        font_size = 1.5
        thick = 3
        cv2.putText(img, 'image', (int(1 / 3 * img.shape[1]), int(0.05 * img.shape[0])), cv2.FONT_HERSHEY_SIMPLEX,
                    font_size, [51, 51, 255], thick, 2)
        cv2.putText(mesh_render, 'rendered SMPL-X mesh',
                    (int(1 / 5 * mesh_render.shape[1]), int(0.05 * mesh_render.shape[0])), cv2.FONT_HERSHEY_SIMPLEX,
                    font_size, [51, 51, 255], thick, 2)
        cv2.putText(render, 'render', (int(1 / 3 * render.shape[1]), int(0.05 * render.shape[0])),
                    cv2.FONT_HERSHEY_SIMPLEX, font_size, [51, 51, 255], thick, 2)

        out = np.concatenate((img, mesh_render, render), 1).astype(np.uint8)
        out = cv2.putText(out, str(frame_idx), (int(out.shape[1] * 0.05), int(out.shape[0] * 0.05)),
                          cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 255), 2, 2)
        video_out.write(out)
